...
final class Bot {
	...
	public function __construct( string $token = '', ?callable $http = null ) {
		...
		$this->http  = $http ?? array( $this, 'transport' );
	}
	...
	public function answerCallbackQuery( string $callback_query_id, ?string $text = null, bool $show_alert = false ): array {
		$params = array( 'callback_query_id' => $callback_query_id, 'show_alert' => $show_alert );
		if ( null !== $text && '' !== $text ) {
			$params['text'] = $text;
		}
	...
	public function transport( string $method, array $params ): array {
		$token = $this->token_set() ? $this->token : (string) get_option( 'trade_telegram_bot_token', '' );
